 ...
 * @param {boolean} include_transactions - Include transactions in sql request
 *
...
async function get_account_transactions(account_id, offset = 0, limit = 25) {
	const transactions = await prisma.transaction.findMany({
		...
		where: {
			OR: [{ from_id: account_id }, { to_id: account_id }],
		},
		orderBy: [{ id: 'desc' }],
	});
...
async function balance(account_id) {
	...
	// validate
	if (!account) throw error('account_not_exist', `Account doesn't exist.`);
...
async function transfer(from_id, to_id, amount, description) {
	...
	if (from_id === to_id) throw error('invalid_transaction', `Such a transaction cannot be completed!`);

	if (!from || !to) throw error('account_not_exist', `Account doesn't exist.`);

	if (from.ban) throw error('account_blocked', 'Your account is blocked!');

	if (to.ban) throw error('account_blocked', `The recipient's account has been blocked!`);

	if ((await balance(from_id)) < amount) throw error('insufficient_funds', `Insufficient funds!`);
...
async function payment(from_id, to_id, amount, description) {
	...
	if (from.ban) throw error('account_blocked', `The buyer's account has been blocked!`);

	if (to.ban) throw error('account_blocked', `Your account is blocked!`);
...
async function verify_payment(confirming_id, transaction_id, code) {
	...
	// validate transaction
	if (!transaction) throw error('transaction_not_exist', `Transaction doesn't exist.`);
...
async function emission(to_id, amount, description) {
	...
	if (!to) throw error('account_not_exist', `Account doesn't exist`);

	if (to.ban) throw error('account_blocked', 'Account is blocked!');
...
async function commission(from_id, amount, description) {
	...
	if (!from) throw error('account_not_exist', `Account doesn't exist`);

	if (from.ban) throw error('account_blocked', 'Account is blocked!');
