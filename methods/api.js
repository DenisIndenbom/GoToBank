...
async function req_auth(req, res, next) {
	...
	if (!req.headers.authorization) return res.status(401).json({ state: 'error', error: 'Token not found.' });
	...
	if (account && !account.ban) {
		...
		return res.status(401).json({ state: 'error', error: 'The token is invalid or the account is blocked.' });
}
